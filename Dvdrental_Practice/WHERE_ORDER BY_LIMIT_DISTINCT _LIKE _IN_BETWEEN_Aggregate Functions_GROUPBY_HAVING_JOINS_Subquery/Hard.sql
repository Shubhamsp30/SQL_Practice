...
select * from rental;
select * from film as f LEFT join inventory as i on f.film_id=i.film_id 
...
group by customer_id 
having sum(amount)>(select avg(total)from(select sum(amount)as total from payment GROUP by customer_id))

-- Show film category with highest number of films
select * from film;
select * from film_category;
SELECT * from category;
select fc.category_id,c.name,count(film_id) films from 
...
select * from customer;
select * from rental;
select c.first_name,r.customer_id,count(r.rental_id) as highest_rental from 
rental as r join customer as c on c.customer_id=r.customer_id 
...
group by customer_id 
having count(rental_id)>(select avg(cnt)from (select count(rental_id)as cnt from rental group by customer_id)1)
...
select f.film_id,count(r.rental_id) as Count_rental 
